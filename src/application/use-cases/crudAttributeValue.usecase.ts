...
export class CreateAttributeValueUseCase {
  ...
  execute(data: CreateAttributeValueInput): AttributeValue {
    ...
    const values = this.attributeValueRepo.getAttributeValuesByTenant(data.tenantId);
    const code = data.code.trim().toUpperCase();
    const repeatedInType = values.some(
      (value) =>
        value.attributeTypeId === data.attributeTypeId && value.code.toUpperCase() === code,
    );
    if (repeatedInType) {
      throw new Error("Ya existe un valor con ese código para este tipo de atributo.");
    }

    const hexColor =
      attributeType.inputType === "color" ? normalizeHex(data.hexColor) : undefined;
    if (attributeType.inputType === "color" && hexColor && !isHexValid(hexColor)) {
...
export class UpdateAttributeValueUseCase {
  ...
  execute(data: UpdateAttributeValueInput): AttributeValue {
    ...
    const nextAttributeTypeId = data.attributeTypeId ?? attributeValue.attributeTypeId;
    const attributeType = this.attributeTypeRepo.getAttributeTypeById(
      data.tenantId,
      nextAttributeTypeId,
    );
    ...
    const nextHexColor =
      attributeType.inputType === "color"
        ...
        : undefined;
    if (attributeType.inputType === "color" && nextHexColor && !isHexValid(nextHexColor)) {
      throw new Error("El color hexadecimal no es válido.");
    }
    ...
    return (
      this.attributeValueRepo.getAttributeValueById(data.tenantId, data.attributeValueId) ??
      attributeValue
    );
  }
}
...
export class ListAttributeValuesUseCase {
  ...
  execute(tenantId: string, options?: ListAttributeValuesOptions): AttributeValue[] {
    ...
    return this.attributeValueRepo
      ...
      .filter((attributeValue) =>
        attributeTypeId ? attributeValue.attributeTypeId === attributeTypeId : true,
      )
