...
export function ClientsActiveTable({
  ...
  data: z.infer<typeof clientAllSchema>[];
  table: TanstackTable<z.infer<typeof clientAllSchema>>;
}) {
              ...
              {table.getRowModel().rows?.length ? (
                ...
                >
                  {table.getRowModel().rows.map((row) => (
                    <DraggableRowAll key={row.id} row={row} />
                  ))}
                </SortableContext>
              ) : (
                <TableRow>
                  <TableCell
                    colSpan={columnsClientAll.length}
