...
export function PayrollListView({ payrolls, onPayrollsChange }: PayrollListViewProps) {
  ...
  const handleRecalculate = (payroll: Payroll) => {
    ...
    onPayrollsChange(payrolls.map(p => p.id === payroll.id ? updatedPayroll : p));
  };
  ...
  const getStatusBadge = (status: string) => {
    const variants = {
      draft: { variant: 'secondary' as const, label: 'Borrador', icon: '📝' },
      calculated: { variant: 'default' as const, label: 'Calculado', icon: '✅' },
      paid: { variant: 'success' as const, label: 'Pagado', icon: '💰' },
    };
  ...
  const columns: ColumnDef<Payroll>[] = [
    ...
    },
    {
      ...
      header: 'Extras',
      cell: ({ row }) => formatCurrency(row.original.calculations.overtimeAmount),
    },
    {
      accessorKey: 'calculations.deductions.total',
      header: 'Descuentos',
      cell: ({ row }) => formatCurrency(row.original.calculations.deductions.total),
    },
    {
      ...
      {showDetailModal && selectedPayroll && (
        <PayrollDetailModal
          ...
          onUpdate={(updatedPayroll) => {
            onPayrollsChange(payrolls.map(p => p.id === updatedPayroll.id ? updatedPayroll : p));
          }}
        />
      )}
    </>
